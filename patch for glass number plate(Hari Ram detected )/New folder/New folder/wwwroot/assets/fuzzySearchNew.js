 ...
 * @param param the key of the node to filter from, Current Acceptable Values: name, id; default "name"
 * @returns
 */
function fuzzySearchParam(
  zTreeId,
  searchField,
  isHighLight,
  isExpand,
  ...
  var nameKey = param ? param : zTreeObj.setting.data.key.name; //get the key of the node name
  ...
  function ztreeFilter(zTreeObj, _keywords, callBackFunc) {
    ...
    function filterFunc(node) {
      if (param == 'id' && node && node.id) {
        node[nameKey] = node.id; //recover oldname of the node if exist
      } else if (node && node.oldname && node.oldname.length > 0) {
